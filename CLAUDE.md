...
- **In chat** (finsharpe-agents#282, `components/TurnCredits.tsx`, mounted by `messages/ai.tsx` under the answer card). `readCreditsCarrier` reads `additional_kwargs.credits` and nothing else on it but `charge_minor`: `{kind:"charge"}` draws the charge label — `0.42 credits`, two decimals, the word always, `0.00 credits` for a zero — muted and right-aligned under the card, outside the bubble and never in the hover-only `CommandBar`, linking to the Credits page; `{kind:"refused"}` (a Short Balance refusal) draws `ShortBalanceNotice` with Request credits beside the refusal's ordinary answer card. **No client switch**: the agents decide who gets a carrier (staff alone in Shadow Mode, every account once enforced; a refusal only when enforced), so an answer without one shows nothing. The composer never reads credits and stays live after a refusal. Mirrors finsharpe-mobile's #281.
