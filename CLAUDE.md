...
- Deleting the account, and signing out (`AuthProvider`'s `logout`), clear what the browser kept about the user's connections — the persisted query cache (the IndexedDB copy of connected financial data) and every `moneyone:*` consent record the old web build left in localStorage — once the request settles, whatever its outcome (finsharpe-agents#283). Revoking a connection (`useRevokeConsent`) removes that connection's data from the copy and its `moneyone:*` record, on every outcome too. Both go through `src/lib/browser-copies.ts`. The copy is kept only while the `user_info` cookie says someone is signed in, and only for the account that wrote it (the cookie's `id`, recorded in the copy as `owner`): another open tab cannot write it back, a page that starts signed out, or under another account, removes any copy it finds, as it does one that names no account, and a page writes nothing more once another account signs in, even in the same tab; nor does a tab write back a connection that left the copy after it read it (a revoke in another tab). A page that starts signed out also removes every `moneyone:*` record. The copy holds successful `["fi-data", …]` queries and never a mutation, paused or not (`COPY_DEHYDRATE_OPTIONS`): the library's default would save a request started offline, a sign-in or a revoke, with its arguments. `src/lib/query-persistence.ts` builds the persister and clears it, and is the only module that may reach IndexedDB; `src/lib/legacy-consent-store.ts` is the only one that names a `moneyone:` key. Checks: `pnpm check:fi-cache`.
...
- `SidebarAccountFooter` carries, signed in, a Credits row (coins glyph, the Balance trailing; an icon on the collapsed rail), the light/dark switch and the identity row — the name opens Profile, a separate button signs out — or a Login button when signed out. On mobile the header avatar opens the same actions as a bottom sheet (`MobileAccountMenu`, after finsharpe-mobile's `account_sheet.dart`), with the Credits row under the identity row; the bottom tab bar holds navigation only. MCP Access and Delete Account are not in the nav — they are on the Profile page, with a Credits row beside MCP Access that links to the Credits page. The Balance figure trails the footer's and the sheet's Credits rows only, never Profile's (owner, 2026-09-25).
...
### account module
`src/modules/account/` holds `ProfileSettingsPage` (real user data only — name, email, roles — then a Credits row (a link, no figure) and MCP Access in one card, sign out and, last, a destructive-but-quiet Delete account; the rows reuse `useAccountActions`' handlers and close the overlay before leaving). There is no Account Settings screen and no Assistant Mode: identity, sign out and the theme switch live in the shell's nav (see the shell module); Credits, MCP Access and Delete Account live on Profile.

### credits module
`src/modules/credits/` is Credits on the web (finsharpe-agents#280, on the user API #269: `GET /api/me/credits`, `GET /api/me/credits/history`; generated as `credits-apis` from the `me-credits` tag). The page is `/settings/credits`, on the `/settings/mcp` pattern: the Balance headline, the state line, Request credits, then the flat History with "Show older".
- **Units.** The wire speaks hundredths (`*_minor`). `utils/format.ts` is the one place credits become words: a Balance to two decimals with a true minus (U+2212), never floored at zero; a price whole; a History amount signed. **No surface shows a USD figure, a rate or a token count** — `pnpm check:credits` scans the copy, the rendered markup and the generated contract for them.
- **State line.** Nothing at or above zero; "Your balance is below zero." when negative and not `gated`; the Short Balance notice when `gated` — the server's verdict, never inferred from the figure.
- **Request credits** is a prefilled `mailto:` to `SUPPORT_EMAIL` (`utils/request.ts`), offered at any Balance on the page and, from #282, beside a refused answer (`ShortBalanceNotice`) — nowhere else. A line whose value is unknown is left out.
- **History rows** (`utils/history.ts`): chat rows titled from the thread cache, "Chat" when the thread is gone; qualifiers for an open turn's window, a stopped / timed-out / errored turn, waived and refunded rows; Charges in ink, Allotments and Refunds in green; no running Balance (J8). Times are IST.
- **Refetch.** Every Balance or History observer refetches on mount (an account surface or the page opening); `useRefreshCreditsOnCarrier` (mounted in the chat thread) refetches when a `credits` carrier lands on an answer; a Pipeline purchase refetches whatever it was answered. The copy mirrors finsharpe-mobile's Credits screen (#231); change the two together. Checks: `pnpm check:credits`.
- **In chat** (finsharpe-agents#282, `components/TurnCredits.tsx`, mounted by `messages/ai.tsx` under the answer card). `readCreditsCarrier` reads `additional_kwargs.credits` and nothing else on it but `charge_minor`: `{kind:"charge"}` draws the charge label — `0.42 credits`, two decimals, the word always, `0.00 credits` for a zero, nothing for a negative or non-whole figure — muted (`text-slate-500`, AA under the navy remap) and right-aligned under the card, outside the bubble and never in the hover-only `CommandBar`, linking to the Credits page, with the accessible name "This answer used 0.42 credits"; `{kind:"refused"}` (a Short Balance refusal, keyed on this carrier and never on the Refusal Marker) draws `ShortBalanceNotice` with Request credits beside the refusal's ordinary answer card — only while the refusal is the thread's latest turn; once a later turn exists it is the answer card alone (owner, 2026-09-25). **No client switch**: the agents decide who gets a carrier (staff alone in Shadow Mode, every account once enforced; a refusal only when enforced), so an answer without one shows nothing. The composer never reads credits and stays live after a refusal. Mirrors finsharpe-mobile's #281.
...
- `chat/` — composer, empty state, toolbar, model picker (the real tiers in `src/configs/models.ts`), Hear Output. Every send goes through `useChatSubmit`; `usePendingPromptHandoff` sends a prompt handed over from another page. A send that cannot connect, an answer that stops early and a chat that cannot load each say so in the thread with a Retry (`components/thread/messages/stream-error.tsx`). A run the agents end with `GuardrailUnavailableError` or `CreditsUnavailableError` (the class name in the runtime's SSE error frame, read by `src/lib/stream-error.ts`, never the server's message) says "FinSharpe is briefly unavailable." instead — finsharpe-mobile's `SharedCopy.serviceBrieflyUnavailable` word for word, full stop included; change the two together — wherever the thread stopped (finsharpe-agents#282); the class names are a wire contract with finsharpe-agents, as in finsharpe-mobile's `_errorCopyByCode`. Checks: `pnpm check:chat-errors`.
...
- `pipelines/` — Agent Workflows (`/discover/research/**`): catalog, quote, run, report, library, shared report. `utils/errors.ts` decides "the server answered and refused" vs "we never reached it"; only the first may state a fact about the thing being asked for.
  - Prices and Balances are the minor-unit fields (#251), labelled by the credits module's formatters; affordability is the server's `can_afford` / `shortfall_minor`, never re-derived. The purchase repeats the quoted `price_minor`, and its structured refusals are drawn on the quote, not toasted (`utils/purchase-refusal.ts`): a 402 re-draws the quote short from the Balance it read (Balance in rose, "You need N credits more… from the Credits screen.", said aloud through the quote's polite live region, Run at 40%), a 409 `price_changed` replaces the quote with the fresh one it carries, a 503 `purchases_unavailable` says "briefly unavailable" in our words. Checks: `pnpm check:credits`.
- `import-data/` — the Import page, the connected accounts list and the analysis modals. The Account Aggregator flow goes through the backend's `/api/aa/*` under the session JWT, the same transport finsharpe-mobile uses — see below.
  ...
  - The Deep Dive rows mirror finsharpe-mobile's `_SmartAlertsSection` (#140 / #145): News draws the shared discover `NewsCard` (the feed mapped by `utils/holdings-news.ts`, after `asMarketNewsItem`); Technical and Fundamental draw `SignalCard` (symbol, verdict chip, served line, and an `Ask AI →` link, the only live part). Their prompts are `technicalSignalMessage` / `fundamentalSignalMessage` in `utils/chat-handoffs.ts`, byte for byte with mobile's. A loaded, empty row renders nothing; a failed one says so.
  - News paging (finsharpe-mobile #174): the News row shows See all when the feed returns a `nextCursor`, opening `/import/news` (`HoldingsNewsPage`). Page 1 is the row's own cached request; later pages re-send its body with `cursor`, and the end line names `quietHoldings` by symbol. Home's Latest Market News See all opens Discover's Market news, which pages `GET /api/news/market/list`. Both lists share `PagedNewsList` + `useNewsFeed` over `NewsFeedPager` (`discover/utils/news-feed.ts`, after mobile's `news_feed.dart`): dedup on id, up to 3 empty pages chained before Show more, a failed page keeps its cards behind Try again. Checks: `pnpm check:chat-handoffs`.
